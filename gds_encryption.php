...
class GDS_Encryption_Class
{
	...
	public function save_to_remote($value, $lead_id, $group="")
	{
		...
		if(!empty($options['use_remote_storage']))
		{
			...
			if($link)
			{
				...
				$prepared_sql = $wpdb->prepare( "INSERT INTO " . mysqli_real_escape_string($link, $options['remote_database_table']) . " (`" . mysqli_real_escape_string($link, $options['remote_database_table_value']) . "`, `" . mysqli_real_escape_string($link, $options['remote_database_table_parent_id']) . "`, `" . mysqli_real_escape_string($link, $options['remote_database_table_group']) . "`) VALUES ( %s, %s, %s )", $values );
				
				if (mysqli_query($link, $prepared_sql))
				{
	...
	public function get_from_remote($id)
	{
		...
		if($link)
		{
			...
			$prepared_sql = $wpdb->prepare( "SELECT `" . mysqli_real_escape_string($link, $options['remote_database_table_value']) . "` FROM " . mysqli_real_escape_string($link, $options['remote_database_table']) . " WHERE `" . mysqli_real_escape_string($link, $options['remote_database_table_id']) . "` = %d", mysqli_real_escape_string($link, $id) );

			if($result = mysqli_query($link, $prepared_sql)) 
			{
	...
	public function delete_entry($parent_id, $group="")
	{
	    ...
	    if(!empty($options['remote_database_removal']))
	    {
		    ...
		    if($link)
		    {
				...
				$prepared_sql = $wpdb->prepare( "DELETE FROM " . mysqli_real_escape_string($link, $options['remote_database_table']) . " WHERE `" . mysqli_real_escape_string($link, $options['remote_database_table_parent_id']) . "` = %d AND `" . mysqli_real_escape_string($link, $options['remote_database_table_group']) . "` = %s", $values );
				
		    	mysqli_query($link, $prepared_sql);
		    }
	    }
	}
}
